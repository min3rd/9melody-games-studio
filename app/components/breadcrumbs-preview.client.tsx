...
export default function BreadcrumbsPreview(): React.ReactElement {
  ...
  const snippet = (
    <CodePreview code={`<Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Breadcrumbs', active: true }]} ${useCustom ? `color={"${color}"} ` : `preset={"${preset}"} `}size="${size}" textSize="${textSize}" showIcon={${showIcon}} rounded={${rounded}} separatorVariant="${separatorVariant}" />`} />
  );

  return (
    <PreviewLayout title="Breadcrumbs" preview={preview} controls={controls} snippet={snippet} />
  );
}
