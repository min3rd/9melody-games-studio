...
export default function ProgressPreview(): React.ReactElement {
  ...
  const preview = (
    <div className="p-4 bg-white dark:bg-neutral-800 rounded">
      <p className="mb-2 text-sm">{t('preview.progress.description') ?? 'Interactive progress component preview.'}</p>
      <div className="mb-2 text-sm">{t('preview.progress.value')}: {value}</div>
  ...
  const snippetCode = `import { Progress } from '@/components/ui';\n\n<Progress value={${value}} indeterminate={${indeterminate}} size="${size}" preset="${preset}"${useCustom ? ` color="${color}"` : ''} rounded="${rounded}" withEffects={${withEffects}} labelPosition="${labelPosition}" />`;
  ...
  return (
    <PreviewLayout title="Progress" preview={preview} controls={controls} snippet={snippet} />
  );
}
