...
import { Kbd, CodePreview } from '@/components/ui';
...
export default function KbdPreview(): React.ReactElement {
  ...
  const snippet = (
    <CodePreview code={`<Kbd ${preset !== 'none' ? (preset === 'custom' ? `color="${color}"` : `preset="${preset}"`) : ''} size="${size}" rounded={${rounded}} withEffects={${withEffects}}>Ctrl</Kbd>`} />
  );

  return (
    <PreviewLayout title="Kbd" preview={preview} controls={controls} snippet={snippet} />
  );
}
