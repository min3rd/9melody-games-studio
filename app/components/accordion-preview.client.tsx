  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 items-center">
        ...
        <label className="text-sm ml-2"><input type="checkbox" checked={withIcon} onChange={(e) => setWithIcon(e.target.checked)} /> Icon</label>
      </div>
      ...
      >
        <div className="text-sm text-neutral-700 dark:text-neutral-300">This is the content of the accordion. Add any components inside.</div>
      </Accordion>

      <CodePreview code={`<Accordion title={"Accordion Title"} ${preset === 'custom' ? `color={"${color}"}` : (preset !== 'none' ? `preset={"${preset}"}` : '')} ${disabled ? 'disabled ' : ''}>...</Accordion>`} />
    </div>
  );
}
