...
import { Drawer, DrawerContainer, DrawerContent, DrawerHeader, DrawerBody, DrawerFooter } from "@/components/ui";
...
export default function DrawerPreview(): React.ReactElement {
  ...
  return (
    <PreviewLayout
      ...
      preview={
        ...
        >
          <Drawer aria-label="Example Drawer">
            <DrawerHeader>
              <div className="flex items-center justify-between w-full">
                <div className="text-sm font-medium">Example Drawer</div>
                <div className="flex items-center gap-2">
                  <button className="px-2 py-1 text-xs rounded-sm" onClick={() => setOpen(false)}>✕</button>
                </div>
              </div>
            </DrawerHeader>
            <DrawerBody>
              <p className="text-sm text-neutral-700 dark:text-neutral-300">
                This is an example of a drawer content. Try changing position and
                sizes.
              </p>
            </DrawerBody>
            <DrawerFooter>
          ...
          <DrawerContent>
            ...
            <p className="text-sm text-neutral-700 dark:text-neutral-300">
              This is the main content. Drawer mode {mode}; position {position}
            </p>
          </DrawerContent>
        </DrawerContainer>
      }
      ...
      snippet={
        <CodePreview
          code={`<DrawerContainer open={open} onClose={() => setOpen(false)} position="${position}" mode="${mode}" width={${width}} height={${height}} ${
            useCustom ? `color="${color}"` : `preset="${preset}"`
          }>
      <Drawer />
      <DrawerContent />
    </DrawerContainer>`}
