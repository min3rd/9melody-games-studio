...
export default function AlertPreview(): React.ReactElement {
  ...
  const snippet = (
    <CodePreview code={`<Alert title={"Heads up"} description={"This is an alert message to notify the user about something important."} ${useCustom ? `color={"${color}"} ` : `preset={"${preset}"} `}variant={"${variant}"} size={"${size}"} rounded={"${rounded}"} withEffects={${withEffects}} dismissible={${dismissible}} ${showAction ? `action={{ label: "Fix", preset: "${preset}" }}` : ''} />`} />
  );

  return (
    <PreviewLayout title="Alert" preview={preview} controls={controls} snippet={snippet} />
  );
}
