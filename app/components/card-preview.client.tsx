...
export default function CardPreview() {
  ...
  const snippet = (
    <CodePreview code={`<Card size="${size}" ${preset !== 'none' ? (preset === 'custom' ? `color="${color}"` : `preset="${preset}"`) : ''} rounded={${rounded}} withEffects={${withEffects}} elevation="${elevation}">...</Card>`} />
  );

  return (
    <PreviewLayout title="Card" preview={preview} controls={controls} snippet={snippet} />
  );
}
