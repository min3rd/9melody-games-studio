...
export default function MenuPreview(): React.ReactElement {
  ...
  const snippet = (
    <CodePreview code={`<Menu items={/* items */} size="${size}" ${preset ? `preset="${preset}" ` : ''}${useCustom ? `color="${color}" ` : ''}withEffects={${withEffects}} />`} />
  );

  return (
    <PreviewLayout title="Menu" preview={preview} controls={controls} snippet={snippet} />
  );
}
