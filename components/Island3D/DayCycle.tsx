...
export default function DayCycle({ cycleDuration = 60 }: { cycleDuration?: number }) {
  ...
  function smoothLerp(a: string, b: string, t: number) {
    const easedT = t * t * (3 - 2 * t); // smoothstep easing
    const ca = new THREE.Color(a);
    const cb = new THREE.Color(b);
    return ca.lerp(cb, easedT);
  }
  ...
  useFrame((state, delta) => {
    ...
    if (t < 0.15) {
      ...
      const p = t / 0.15; // 0..1
      sky = smoothLerp(skyNight, skyDawn, p);
      sunColor = smoothLerp(sunNight, sunDawn, p);
      ambColor = smoothLerp(ambNight, ambDawn, p);
      ...
      sky = smoothLerp(skyDawn, skyDay, nt);
      sunColor = smoothLerp(sunDawn, sunDay, nt);
      ambColor = smoothLerp(ambDawn, ambDay, nt);
      ...
      sky = smoothLerp(skyDay, skyDay, nt); // constant
      sunColor = smoothLerp(sunDay, sunDay, nt);
      ambColor = smoothLerp(ambDay, ambDay, nt);
      ...
      sky = smoothLerp(skyDay, skyDusk, nt);
      sunColor = smoothLerp(sunDay, sunDusk, nt);
      ambColor = smoothLerp(ambDay, ambDusk, nt);
      ...
      sky = smoothLerp(skyDusk, skyNight, nt);
      sunColor = smoothLerp(sunDusk, sunNight, nt);
      ambColor = smoothLerp(ambDusk, ambNight, nt);
      dirIntensity = 0.2;
      ambIntensity = 0.05;
      starIntensity = 1.0;
    }
    ...
    if (starsRef.current) {
      try {
        ...
        const factor = 1 - Math.exp(-speed * delta); // exponential smoothing
        const next = THREE.MathUtils.lerp(prev, starIntensity, factor);
        starOpacityRef.current = next;

        starsRef.current.visible = next > 0.01; // visibility threshold
        if (starsRef.current.material) {
          ...
          // Smooth vertical rise/fall effect
